namespace Athkar.Shareds.Hosting;

/// <summary>
/// The plumbing every sweeper needs, so none of them has to own it: a timer, a
/// fresh dependency-injection scope per pass, a sweep before the first tick, and
/// a failure that costs one pass rather than the worker.
///
...
/// construction would pin a single context open for the life of the process and
/// let its change tracker grow forever.
/// </summary>
public abstract class PeriodicWorker : BackgroundService
{
    ...
    /// <summary>
    /// How often to look. The granularity of the sweep, not a product decision —
    /// the deadlines themselves are configuration — so it only has to be short
    /// enough that "due" and "done" feel simultaneous.
    /// </summary>
    ...
    /// <summary>
    /// One pass, against the given scope. Returns how many rows it touched; zero
    /// is the ordinary answer and is not logged.
    /// </summary>
